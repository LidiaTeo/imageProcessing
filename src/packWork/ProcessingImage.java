...
public class ProcessingImage extends SuperClass1{
	...
	public ProcessingImage(Buffer b) {
		this.b = b;
	}

	public void run () {
		b.setImage(b.consume());		
		this.overwritePixel();

		System.out.println("Consumatorul a procesat imaginea. ");
...
public void overwritePixel() {
	...
	int width = b.getImage().getWidth(); //salvez latimea imaginii
	int height = b.getImage().getHeight(); //salvez inaltimea imaginii
	...
	for( int i = 0; i < width; i++) { //parcurgem matricea pe coloane
		for (int j = 0; j < height; j++) {
			pixel = b.getImage().getRGB(i,j); //se obtine pixel-ul de la pozitia curenta
			alpha = (pixel >> 24) & 0xff; //shiftez bitii pentru a separa cele 3 valori ale culorilor primare: rosu, verde si albastru
			red = (pixel >> 16) & 0xff;
			green = (pixel >> 8) & 0xff;
			blue = pixel & 0xff;

			greyscale = (int)(0.2989*red) + (int)(0.5870*green) + (int)(0.1140*blue); //calculam valoarea cu care vor fi suprascrise 
			//valorile celor 3 culori primare ale pixelui curent.

			pixel = (alpha << 24) | (greyscale << 16) | (greyscale << 8) | greyscale; //scriem noile valori inapoi in pixel-ul curent
			b.getImage().setRGB(i,j,pixel); //aplicam modificarile efectuate pixel-ului.
		}
	}
